...
# documentation: https://docs.python.org/3/library/unittest.html#unittest
# .TestCase
class TestPS4BC(unittest.TestCase):
    ...
    def setUpPlaintext(self):
        self.pads = [
            [71, 12, 44, 87, 81, 30, 61, 5, 66, 19, 73, 91, 19, 8, 83],
            [
                34,
                92,
                62,
    ...
    def test_message_get_text(self):
        ...
        self.assertEqual(
            response,
            self.text1,
            f"get_text returned {response}, but {self.text1} was expected",
        )
        response = self.msg2.get_text()
        self.assertEqual(
            response,
            self.text2,
            f"get_text returned {response}, but {self.text2} was expected",
        )
        response = self.msg3.get_text()
        self.assertEqual(
            response,
            self.text3,
            f"get_text returned {response}, but {self.text3} was expected",
        )
    ...
    def test_message_shift_char(self):
        ...
        for char, shift, exp in shifts:
            ...
            self.assertEqual(
                exp,
                result,
                f"Shifting the character '{char}' by {shift} resulted in "
                f"'{result}' but '{exp}' was expected",
            )
    ...
    def test_message_apply_pad(self):
        pads = [
            (
                self.msg1,
                "testing message",
                [84, 69, 6, 5, 21, 25, 1, 79, 35, 54, 35, 45, 60, 83, 45],
                "iKyy~(ho1<7A>[3",
            ),
            (
                self.msg2,
                "A m4ssage with punctuation in it+... Fun!",
                [
                    1,
                    56,
                    14,
                ...
                'BX{BQ>s"1_s T`mZ\'G0=w&"-]nf1qw;{$T>tXsDn:',
            ),
            (
                self.msg3,
                "zzzzz~~~~....",
                [62, 30, 73, 77, 42, 11, 13, 51, 68, 79, 25, 90, 9],
                "Y9dhE*,Rc}G)7",
            ),
        ...
        for m, msg, pad, exp in pads:
            ...
            self.assertEqual(
                exp,
                result,
                f"Applying a pad to '{msg}' resulted in '{result}' but '{exp}' was expected",
            )

    def test_plaintext_message_inheritance(self):
        self.setUpPlaintext()
        for plaintext, text in zip(
            (self.ptmsg1, self.ptmsg2, self.ptmsg3, self.ptmsg4),
    ...
    def test_plaintext_message_generate_pad(self):
        self.setUpPlaintext()
        plaintexts = (
            (self.ptmsg4, [108, 49, 97, 53, 5, 33, 65, 62]),
            ...
            (
                self.ptmsg1,
                [108, 49, 97, 53, 5, 33, 65, 62, 51, 100, 106, 38, 61, 45, 74],
            ),
        )
        ...
        for p, exp in plaintexts:
            ...
            self.assertEqual(
                len(exp),
                len(result),
                f"The length of the generated pad is {len(result)} but " 
                f"{len(exp)} was expected",
            )
            self.assertEqual(
                exp, result, f"The generated pad is {result} but "
                f"{exp} was expected"
            )
    ...
    def test_plaintext_message_pad(self):
        ...
        pds = [
            (
                self.ptmsg1,
                self.pads[0],
                [63, 34, 46, 81, 31, 88, 60, 42, 10, 68, 40, 28, 86, 71, 10],
            ),
            (
                self.ptmsg2,
                self.pads[1],
        ...
        for p, init_pad, set_pad in pds:
            ...
            self.assertEqual(
                init_pad,
                result,
                f"The PlaintextMessage was initialized with the pad "
                f"{init_pad} but get_pad() returned {result}",
            )
            ...
            self.assertEqual(
                set_pad,
                result,
                f"The PlaintextMessage pad was changed to "
                f"{set_pad} but get_pad() returned {result}",
            )
        ...
        self.assertEqual(
            exp,
            result,
            f"The randomly generated PlaintextMessage pad was {result} but "
            f"{exp} was expected",
        )
    ...
    def test_plaintext_message_ciphertext(self):
        ...
        pds = [
            ...
            (
                self.ptmsg2,
                "c|Lo4ovYdE,V&|'/x+x#Yjp;sf9ugd/'iL}!s.|en",
                [
                    19,
                    44,
                    12,
                ...
                "TLy`<0yo0me@fs(ic9\\gMa252Fyv(X,muh>Po`ZY0",
            ),
            (
                self.ptmsg3,
                "dc,Kz~AGL&G^>",
        ...
        for p, init_enc, new_pad, new_enc in pds:
            ...
            self.assertEqual(
                init_enc,
                result,
                f"The encrypted message returned was {result} but "
                f"{init_enc} was expected",
            )
            p.change_pad(new_pad)
            result = p.get_ciphertext()
            self.assertNotEqual(
                init_enc, result, "change_pad did not update the encrypted "
                "message"
            )
            self.assertEqual(
                new_enc,
        ...
        self.assertEqual(
            exp,
            result,
            f"The PlaintextMessage encrypted message from a randomly generated "
            f"pad was {result} but {exp} was expected",
        )
        pad = self.pads[0]
        ...
        self.assertEqual(
            repr_ptmsg_mutated,
            repr(ptmsg_mutated),
            "Mutating the pad used to initialize PlaintextMessage mutated the "
            "PlaintextMessage object. Did you save pad as a copy?",
        )
    ...
    def test_encrypted_message_inheritance(self):
        for text in (self.text1, self.text2, self.text3, self.text4):
            ...
            self.assertEqual(
                text,
                encrypted.get_text(),
                f"The EncryptedMessage({text})'s get_text() returns "
                f"{encrypted.get_text()} not {text}. Are you calling the super "
                "constructor?",
            )

    def test_encrypted_message_decrypt(self):
        pds = (
            ("Monoo#", [5, 10, 2, 3, 0, 2], "Hello!"),
            (
                "05<Nvv`\\.8_0qvh?b?",
                [59, 44, 50, 90, 86, 13, 13, 60,
                 64, 46, 70, 74, 45, 86, 5, 93, 15, 57],
                "ThiS iS MixED cASe",
            ),
            (
                "8fxvr6/C'9 _^[G4uXCN",
        ...
        for enc, pad, exp in pds:
            ...
            self.assertNotEqual(
                exp,
                enc_msg.apply_pad(pad),
                "Decrypting a message should negate the pad, not just apply it!",
            )
            self.assertEqual(
                exp,
                result,
                f"The decrypted message returned was {result} but "
                f"{exp} was expected",
            )
    ...
    def test_try_pads(self):
        pds = (
            ...
            (
                ...
                "ThiS iS MixED cASe",
                [59, 44, 50, 90, 86, 13, 13, 60, 64, 46,
                 70, 74, 45, 86, 5, 93, 15, 57],
            ),
            (
                "8fxvr6/C'9 _^[G4uXCN",
                [
        ...
        for enc, pads, exp_msg, exp_pad in pds:
            ...
            self.assertEqual(
                exp_pad,
                result_pad,
                f"decrypt_message_try_pads returned {result_pad} as the best "
                f"pad but {exp_pad} was expected",
            )
            self.assertEqual(
                exp_msg,
                result_msg,
                f"decrypt_message_try_pads returned the correct pad and "
                f"{result_msg} as the decrypted story when "
                f"{exp_msg} was expected",
            )
        ...
        self.assertEqual(
            [1, 2, 3],
            result_pad,
            "decrypt_message_try_pads should return the last pad if none "
            "of them decrypt",
        )
        self.assertEqual(
            "a_a",
            result_msg,
            "decrypt_message_try_pads should return the last pad's "
            "decryption if none of them decrypt",
        )
...
point_values = {
    ...
    "test_message_shift_char": 0.5,
    "test_message_apply_pad": 0.5,
    "test_plaintext_message_inheritance": 0.25,
    "test_plaintext_message_generate_pad": 0.5,
    "test_plaintext_message_pad": 0.5,
    "test_plaintext_message_ciphertext": 0.5,
    "test_encrypted_message_inheritance": 0.25,
    "test_encrypted_message_decrypt": 0.25,
    "test_try_pads": 0.5,
}
...
class Results_600(unittest.TextTestResult):
    ...
    def handleDeduction(self, test_name, message):
        ...
        if message is None:
            message = f"Your code produced an error on test {test_name}."
        self.output.append(f"[-{point_value}]: {message}")
